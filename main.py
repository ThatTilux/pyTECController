"""
Right now, this is just the example.py from the pymeCon
"""
import logging
import platform
...
class MeerstetterTEC(object):
    ...
    def _connect(self):
        ...
        if self.port is not None:
            ...
            serialport = base_name

            scan_start_time = time()
            while True:
                for i in range(start_index, MAX_COM + 1):
                    try:
                        serialport = base_name + str(i)
                        self._session = MeComSerial(serialport=serialport)
                        break
        ...
        logging.info("connected to {} with serialport {}".format(self.address, serialport))
